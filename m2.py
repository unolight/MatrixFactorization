...
# RatingCount = np.zeros((M,)) # num of uesrs rate item i
Test = {}
...
class Client:
    ...
    def genNoise(self, j, H):
        self.eta[j] = 2*4*sqrt(K)/epi*sqrt(2*H)*self.C[j]
    def gradient(self, Q):
        self.noise2= Noise()
        # print self.noise2
        grad = initGrad()
        for j in xrange(M):
            if R[0][j] != 0:
                ...
                for k in xrange(K):
                    grad[j][k]=Q[k][j]+alpha*(2*eij*self.p[0][k]-beta*Q[k][j])+self.noise[j][k]+self.noise2[j][k]
        return uid, grad
